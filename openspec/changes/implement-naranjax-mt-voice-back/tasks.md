...
- [x] 1.1 RED — Extend `tests/orchestrator/test_service.py` with truthful
...
- [x] 1.2 GREEN — Add `RunRequest.extras` and `ArtifactRole.ANOMALIES`;
...
- [x] 1.3 VERIFY — `python -m pytest tests -q`; numstat below 400.

## Phase 2: PR2 — Back Adapter, Promotion, E2E

- [x] 2.1 RED — Create `tests/adapters/naranjax/test_mt_voice_back.py`
...
- [x] 2.2 GREEN — Create `adapters/naranjax/mt_voice_back.py`; add the
      seventh catalog entry; register the adapter key. Rerun focused tests.
- [x] 2.3 VERIFY — Full suite; MT legacy suite (10 passed, untouched); one
