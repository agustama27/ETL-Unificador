...
def quitar_15_local_argentino(cuerpo):
    ...
    # Solo evaluar cuerpos largos que podrian incluir el "15" local.
    if len(cuerpo) < 10 or len(cuerpo) > 12:
        return cuerpo, False
...
def generar_csv_telefonos(df, carpeta_base):
    ...
    nombre_archivo = f'telefonos_epec_{fecha_formato}.csv'
...
def guardar_csv_consolidado(df, carpeta_base, nombre_archivo=None):
    ...
    if nombre_archivo is None:
        fecha_actual = datetime.now()
        fecha_formato = fecha_actual.strftime('%d%m%Y')
        nombre_archivo = f'base_epec_{fecha_formato}.csv'
    ...
    df.to_csv(archivo_salida, sep=';', index=False, encoding='utf-8', na_rep='')
