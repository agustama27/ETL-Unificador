...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Generar tambien el archivo CHAT ROMAN (1 fila por DNI)",
    )
    parser.add_argument(
        "--sin_planes_hoy",
        action="store_true",
        default=False,
        help="Indica que la base de planes del dia NO llego (tiene_planes=False para todos)",
    )
    ...
    archivos = ArchivosDia(
        ...
        generar_chat=bool(args.chat),
        planes_disponibles_hoy=not bool(args.sin_planes_hoy),
    )
