...
class SyntheticRunner:
    ...
    def run(self, command, cwd, env, timeout, *, secret_values):
        ...
        run = next(Path(value) for value in command
                   if Path(value).name.startswith("base.")).parents[1]
        if self.mode == "success":
            write_result(run, self.mode, channel="cartasur")
...
def _base(tmp_path: Path, name: str = "base.csv") -> Path:
    path = tmp_path / name
...
def test_cli_selects_cartasur_adapter(tmp_path: Path) -> None:
    service = RecordingService()

    def factory(definition, adapter):
        return service

    assert main(["--etl", CARTASUR, "--fecha", "20260721",
                 "--base", str(_base(tmp_path, "base.xlsx"))],
                adapters=_adapters(), service_factory=factory) == 0
    assert set(dict(service.requests[0].inputs)) == {"base"}
...
def test_cartasur_cli_lifecycle(
    ...
    exit_code = main(["--etl", CARTASUR, "--fecha", day, "--base", str(_base(tmp_path))],
    ...
    if status == "succeeded":
        assert [(item["role"], item["path"]) for item in evidence["artifacts"]] == [
            ("roman", "output/CARTA_SUR_ROMAN_260721.csv"),
            ("e1kia", "output/CARTA_SUR_E1KIA_260721.csv"),
        ]
        assert evidence["postconditions"] == {"outputs": "passed", "state": "not_applicable"}
        assert any(value.endswith("base.csv") for value in runner.command)
