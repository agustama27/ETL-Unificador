...
PAGOS_COLUMN_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "dt_actual": ("dt_actual", "deuda_total_actual", "total_deuda_actual", "monto_deuda_total"),
    "dv_actual": (
        "dv_actual",
        "deuda_vencida_actual",
        "total_vencida_actual",
        "monto_deuda_vencida",
    ),
    "deuda_vencida": (
        "deuda_vencida",
...
def load_pagos(filepath: str) -> pd.DataFrame:
    ...
    for optional_amount_column in ("dt_actual", "dv_actual", "deuda_vencida"):
        if optional_amount_column not in normalized.columns:
            aliases = PAGOS_COLUMN_ALIASES.get(optional_amount_column, (optional_amount_column,))
            found_header = next(
                (
                    positions_by_header[normalized_alias]
                    for alias in aliases
            ...
            if found_header is not None:
                normalized[optional_amount_column] = df[found_header].astype(str).str.strip()
            else:
                normalized[optional_amount_column] = PAGOS_DEFAULT_VALUES.get(optional_amount_column, "")
