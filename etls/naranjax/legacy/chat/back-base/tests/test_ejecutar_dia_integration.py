...
def _load_single_chat_output(output_dir: Path) -> pd.DataFrame:
    outputs = sorted(output_dir.glob("NARANJAX_MA_CHAT_ROMAN_*.csv"))
    assert outputs, "No CHAT ROMAN outputs generated"
    return pd.read_csv(outputs[-1], sep=";", dtype=str, keep_default_na=False)
...
def test_ejecutar_dia_genera_output_chat_y_respeta_sin_planes_hoy(tmp_path: Path) -> None:
    input_workbook = _build_input_workbook(tmp_path)
    diarios_dir = tmp_path / "diarios"
    diarios_dir.mkdir(parents=True, exist_ok=True)
    output_dir = tmp_path / "out"
    ...
    chat = _load_single_chat_output(output_dir)
    assert len(chat) >= 1
    assert "tiene_planes" in chat.columns
    assert set(chat["tiene_planes"]) == {"False"}
