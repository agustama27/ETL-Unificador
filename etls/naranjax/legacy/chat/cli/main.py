...
def run_cli(argv: list[str] | None = None) -> int:
    ...
    cfg = load_config()
    estado_dir = args.estado or cfg.get("carpeta_estado")
    salida_dir = args.salida or cfg.get("carpeta_salida")
    if not estado_dir or not salida_dir:
        print("Falta configurar carpeta de estado/salida (usar config.json o flags --estado/--salida)")
        return 1
    ...
    config = ConfigDia(
        ...
        logs_dir=Path(salida_dir) / "logs",
        procesados_dir=Path(salida_dir) / "procesados",
    )
    ...
    if archivos.pagos is not None:
        print("Aviso: --pagos se recibe por compatibilidad pero no impacta deuda/saldo ni salida.")
