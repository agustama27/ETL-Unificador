...
def _build_output_path(output_dir: Path) -> Path:
    ...
    return output_dir / f"SOCIAL_CHI_CARTERA_{date_str}.csv"
...
def _find_optional_column_name(fieldnames: list[str], candidates: list[str]) -> Optional[str]:
    normalized_map = {_normalize_key(field): field for field in fieldnames}
    for candidate in candidates:
        match = normalized_map.get(_normalize_key(candidate))
        if match:
...
def generate_base(input_path: Optional[Path] = None, output_path: Optional[Path] = None) -> Path:
    ...
    with input_csv.open("r", encoding="utf-8-sig", newline="") as source_file:
        ...
        apellido_column = _find_column_name(source_fieldnames, ["APELLIDO", "Apellido"])
        nombre_column = _find_column_name(source_fieldnames, ["NOMBRE", "Nombre"])
        documento_column = _find_column_name(source_fieldnames, ["DOCUMENTO", "RUT"])
        monto_cuota_column = _find_column_name(source_fieldnames, ["Monto Cuota", "MONTO", "Monto"])
        monto_column = _find_column_name(source_fieldnames, ["MONTO", "Monto"])
        descuento_column = _find_optional_column_name(
            source_fieldnames,
            ["% Descuento", "Porcentaje de ProntoPago", "PORCENTAJE_BECA", "Porcentaje Beca"],
        )
        descuento_pp_column = _find_optional_column_name(
            source_fieldnames,
            ["Porcentaje de ProntoPago", "porcentaje_de_prontopago"],
        )
        dias_mora_column = _find_column_name(source_fieldnames, ["Dias Mora", "DIAS_VENCIDO", "diasvencido"])
        ...
        for extra_field in [
            "monto_deuda",
            "cantidad_cuotas",
            "monto_deuda_descuento",
            "monto_deuda_descuento_pp",
        ]:
        ...
        for row in reader:
            ...
            descuento = _parse_percentage(row.get(descuento_column, "")) if descuento_column else 0.0
            descuento_pp = _parse_percentage(row.get(descuento_pp_column, "")) if descuento_pp_column else 0.0
            ...
            if descuento_pp > 0:
                grouped_rows[documento]["descuento_pp"] = descuento_pp
        ...
        with output_csv.open("w", encoding=OUTPUT_ENCODING, newline="") as target_file:
            ...
            for group in grouped_rows.values():
                ...
                descuento_pp = group["descuento_pp"]
                monto_deuda_descuento = monto_deuda * (1 - (descuento / 100))
                monto_deuda_descuento_pp = monto_deuda * (1 - (descuento_pp / 100))
                ...
                row["monto_deuda_descuento_pp"] = f"{monto_deuda_descuento_pp:.2f}"
