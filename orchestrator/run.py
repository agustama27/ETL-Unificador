...
ServiceFactory = Callable[[ETLDefinition], Service]
...
def _service(definition: ETLDefinition, workspace: Path, adapter: MaChatAdapter) -> RunService:
...
def main(argv: Sequence[str] | None = None, *, service_factory: ServiceFactory | None = None) -> int:
    ...
    adapter = MaChatAdapter()
    definition = Catalog.load(
        workspace / "registry/naranjax.yaml", workspace,
        adapters={"naranjax.ma.chat": adapter},
    )[arguments.etl]
    service = service_factory(definition) if service_factory else _service(definition, workspace, adapter)
