...
def test_load_input_raises_clear_error_without_matching_sheet(tmp_path) -> None:
    ...
    assert "Renombrá" in error


def test_load_input_uses_single_sheet_as_fallback(tmp_path) -> None:
    workbook_path = tmp_path / "input_hoja1.xlsx"
    _write_input_workbook(workbook_path, "Hoja1")
...
def test_load_input_raises_when_multiple_sheets_no_match(tmp_path) -> None:
    workbook_path = tmp_path / "input_multi_no_match.xlsx"
    with pd.ExcelWriter(workbook_path, engine="openpyxl") as workbook:
        _legacy_base_df().to_excel(workbook, sheet_name="Datos", index=False)
        _legacy_base_df().to_excel(workbook, sheet_name="Resumen", index=False)

    with pytest.raises(ValueError, match="Renombrá"):
        load_input(str(workbook_path))
