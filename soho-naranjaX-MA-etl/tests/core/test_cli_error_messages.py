...
def test_cli_accepts_evoltis_pagos_format_without_validation_failure(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    assert code == 0
    out = capsys.readouterr().out
    assert "se proceso archivo de pagos" in out
...
def test_cli_dev_mode_overrides_output_dir(tmp_path: Path, monkeypatch) -> None:
    estado_dir = tmp_path / "estado"
    estado_dir.mkdir()
    monkeypatch.setattr("cli.main.load_config", lambda: {"carpeta_estado": str(estado_dir), "carpeta_salida": "C:/prod/output"})
    ...
    monkeypatch.setenv("NARANJAX_OUTPUT_DIR", r"C:\Users\agustin.tamagusuku\Documents\NARANJAX_MA_PERSISTENCIA_DEVELOP")

    captured: dict[str, object] = {}

    def _fake_procesar_dia(config, _archivos, log_cb=None):
        captured["output_dir"] = config.output_dir
        return ResultadoDia(status="success", fecha="20260501", mes="202605")
    ...
    assert code == 0
    assert str(captured["output_dir"]).replace("\\", "/") == (
        "C:/Users/agustin.tamagusuku/Documents/NARANJAX_MA_PERSISTENCIA_DEVELOP"
    )


def test_cli_flags_have_priority_over_env_and_default(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        "cli.main.load_config",
        lambda: {"carpeta_estado": "C:/OneDrive/estado", "carpeta_salida": "C:/OneDrive/salida"},
    )
    monkeypatch.setenv("NARANJAX_ESTADO_DIR", "C:/env/estado")
    monkeypatch.setenv("NARANJAX_OUTPUT_DIR", "C:/env/salida")
    ...
    def _fake_procesar_dia(config, _archivos, log_cb=None):
        captured["estado_dir"] = config.estado_dir
        captured["output_dir"] = config.output_dir
        return ResultadoDia(status="success", fecha="20260501", mes="202605")
    ...
    assert code == 0
    assert str(captured["estado_dir"]).replace("\\", "/") == "C:/flag/estado"
    assert str(captured["output_dir"]).replace("\\", "/") == "C:/flag/salida"


def test_cli_uses_config_dirs_when_env_not_set(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        "cli.main.load_config",
        lambda: {
            "carpeta_estado": r"C:\Users\agustin.tamagusuku\OneDrive\estado",
            "carpeta_salida": r"C:\Users\agustin.tamagusuku\OneDrive\salida",
        },
    )
    ...
    assert code == 0
    assert str(captured["estado_dir"]).replace("\\", "/") == "C:/Users/agustin.tamagusuku/OneDrive/estado"
    assert str(captured["output_dir"]).replace("\\", "/") == "C:/Users/agustin.tamagusuku/OneDrive/salida"
